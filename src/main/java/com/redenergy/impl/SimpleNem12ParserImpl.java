...
public class SimpleNem12ParserImpl implements SimpleNem12Parser {
	 ...
	 * @throws Exception
	 */
	private void addMeterVolumeToMeterReadsIfStartWith300(String[] recordType, List<MeterRead> meterReads,
			String firstColumn) throws Exception {
		if (CONST_300.equals(firstColumn)) {
			...
			if (quality.equalsIgnoreCase(Quality.A.name()) && quality.equalsIgnoreCase(Quality.E.name())) {
				throw new Exception(
						format("Quality value '%s' is not valid,value should be either 'A' or 'E'", quality));
			}
			// Get the last element from meter reads list and add the meter volume
			MeterRead meterRead = meterReads.get(meterReads.size() - 1);
			MeterVolume meterVolume = new MeterVolume(BigDecimal.valueOf(Double.parseDouble(recordType[2])),
	 ...
	 * @throws Exception
	 */
	private void addMeterReadBlockIfStartsWith200(String[] record, List<MeterRead> meterReads, String firstColumn)
			throws Exception {
